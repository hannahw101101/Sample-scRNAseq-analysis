...
# Differential Expression: Neurons
```{r}
...
# Differential Expression: Muscle
```{r}
# Create a vector mapping cell -> sample and cell -> celltype
meta <- all@meta.data
# Example for Neurons only
muscle_cells <- rownames(meta)[meta$celltype == "Muscle"]
muscles <- subset(all, cells = muscle_cells)

# Aggregate to pseudobulk: sum counts per gene per sample
muscles <- JoinLayers(muscles)
counts <- GetAssayData(muscles, slot = "counts", assay = "RNA")
samples <- muscles$orig.ident
# aggregate using Matrix::rowSums on subsets:
sample_list <- split(colnames(counts), samples)
pb_counts <- sapply(sample_list, function(cols) rowSums(counts[, cols, drop=FALSE]))
# pb_counts is genes x sample matrix
...
EnhancedVolcano(res_df,
    ...
    x = "log2FoldChange",
    y = "padj",
    title = "Pseudobulk DEG: Mutant vs Wildtype Muscles",
    pCutoff = 0.05,
    FCcutoff = 1,
    pointSize = 2.0,
    labSize = 3.5)
