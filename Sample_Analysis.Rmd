...
all$celltype[all$seurat_clusters == 3] <- "Neuron"
...
DefaultAssay(all_dge) <- "RNA"

# Fixing it? 
all_dge <- JoinLayers(all_dge)

# Running FindMarkers
all_dge <- FindMarkers(all_dge, ident.1 = "mt", ident.2 = "wt")

write.csv(all_dge, "findmarkers_all_mt_vs_wt.csv")

# Subset significant genes
all_sig <- all_dge %>% subset(p_val_adj < 0.05)
all_sig %>% head()

# Volcano plot
p <- EnhancedVolcano(all_sig, 
                     row.names(all_sig),
                     x = "avg_log2FC",
                     y = "p_val_adj",
                     title = "FindMarkers DEG All Cells",
                     subtitle = "Mutant vs. WildType")
...
```{r}
neurons_subset <- subset(all, subset = (celltype == "Neuron"))
...
# Fixing it? 
neurons_subset <- JoinLayers(neurons_subset)

# Running FindMarkers
neurons_subset <- FindMarkers(neurons_subset, ident.1 = "mt", ident.2 = "wt")

write.csv(neurons_subset, "findmarkers_neurons_mt_vs_wt.csv")

# Subset significant genes
neurons_sig <- neurons_subset %>% subset(p_val_adj < 0.05)
neurons_sig %>% head()

# Volcano plot
p <- EnhancedVolcano(neurons_sig, 
                     row.names(neurons_sig),
                     x = "avg_log2FC",
                     y = "p_val_adj",
                     title = "FindMarkers DEG All Cells",
                     subtitle = "Mutant vs. WildType")
